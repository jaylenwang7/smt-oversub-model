...
class TestCoreOverhead:
    ...
    def test_available_pcpus_with_overhead(self):
        ...
        power = PowerCurve(100, 300)
        proc = ProcessorConfig(physical_cores=64, threads_per_core=2, power_curve=power, thread_overhead=8)
        assert proc.pcpus == 128
    ...
    def test_available_pcpus_nosmt_with_overhead(self):
        ...
        power = PowerCurve(100, 300)
        proc = ProcessorConfig(physical_cores=48, threads_per_core=1, power_curve=power, thread_overhead=4)
        assert proc.pcpus == 48
    ...
    def test_available_pcpus_clamps_to_zero(self):
        ...
        power = PowerCurve(100, 300)
        proc = ProcessorConfig(physical_cores=8, threads_per_core=1, power_curve=power, thread_overhead=100)
    ...
    def test_server_count_without_overhead_for_comparison(self):
        ...
        power = PowerCurve(100, 300)
        proc = ProcessorConfig(physical_cores=64, threads_per_core=2, power_curve=power, thread_overhead=0)
        workload = WorkloadParams(total_vcpus=1000, avg_util=0.3)
        cost = CostParams(1000, 15000, 400, 0.10, 4 * 8760)
    ...
    def test_overhead_affects_utilization_calculation(self):
        ...
        power = PowerCurve(100, 300)
        proc_no_overhead = ProcessorConfig(64, 2, power, thread_overhead=0)
        proc_with_overhead = ProcessorConfig(64, 2, power, thread_overhead=8)
...
class TestComponentBreakdown:
    ...
    def test_total_per_server_math(self):
        """total_per_server should be per_thread_total + per_server_total."""
        bd = ComponentBreakdown(
        ...
        # per_thread: (10 + 2) * 96 = 1152
        assert bd.per_thread_total_per_server == 1152.0
    ...
    def test_empty_breakdown(self):
        ...
        assert bd.per_thread_total_per_server == 0.0
    ...
    def test_embodied_breakdown_fleet_components(self):
        ...
        # per_thread.cpu_die: 10 * 96 * 5 = 4800
        assert fleet['per_thread.cpu_die'] == 4800.0
...
class TestComponentBreakdownPerVcpu:
    ...
    def test_total_per_server_includes_per_vcpu(self):
        """total_per_server includes per_thread + per_server + per_vcpu."""
        bd = ComponentBreakdown(
            per_thread={'cpu': 10.0},
